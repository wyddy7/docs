...
## Контекст
- **Для кого:** Студенты курса "Периферийные устройства", освоившие основы работы с инструментами разработки в Лабораторной работе №1.
- **Цель:** Изучить архитектуру защищённого режима процессора x86, механизм сегментной адресации, структуру GDT и процесс переключения между реальным и защищённым режимами.
- **Необходимые знания:** Понимание принципов работы процессора, регистров, ассемблера AT&T.
- **Расположение файлов:** Исходный код и скрипты приведены в разделе "Приложение".
...
![Формирование адреса в реальном режиме](image/lab2-protected-mode/real-mode-address-formation.png)
...
![Защищённый режим](image/lab2-protected-mode/protected-mode.png)
...
![Структура селектора](image/lab2-protected-mode/selector-structure.png)
...
![Структура дескриптора](image/lab2-protected-mode/descriptor-structure.png)
...
![Глобальная и локальная дескрипторные таблицы](image/lab2-protected-mode/gdt-ldt.png)
...
![Структура машинной команды](image/lab2-protected-mode/machine-command-structure.png)
...
![Структура кода операции](image/lab2-protected-mode/opcode-structure.png)
...
![Структура байта modR/M](image/lab2-protected-mode/modrm-structure.png)
...
![Пример №1 разбора команды](image/lab2-protected-mode/command-disassembly-example1.png)

Пример №2 разбора команды:

![Пример №2 разбора команды](image/lab2-protected-mode/command-disassembly-example2.png)

Пример №3 разбора команды:

![Пример №3 разбора команды](image/lab2-protected-mode/command-disassembly-example3.png)
...
![Таблица кодов операций](image/lab2-protected-mode/opcode-table.png)

3. Переходим по ссылке "Переход и процедуры". Определяем команду

![Команда перехода](image/lab2-protected-mode/jump-command.png)
...
- [ ] Созданы файлы `lab2.S`, `linker.ld`, `Makefile`
- [ ] Программа успешно собирается через `make`
- [ ] Программа запускается в QEMU, наблюдается вывод символов и возврат в реальный режим
- [ ] Выполнено изменение сообщения в коде
- [ ] Проведен анализ регистров и GDT в отладчике GDB
- [ ] Выполнены эксперименты с селекторами (несуществующий, неправильный, алиас)
- [ ] Выполнены задания по защите лабораторной работы

## Часто задаваемые вопросы (FAQ)

**Q: Почему мы используем `ljmp` для перехода в защищённый режим?**
A: Инструкция `ljmp` (long jump) позволяет одновременно обновить регистр CS (сегмент кода) и EIP (указатель инструкции). При переходе в защищённый режим в CS нужно загрузить селектор сегмента кода из GDT, что невозможно сделать командой `mov`.

**Q: Зачем нужен макрос `descr`?**
A: Структура дескриптора сегмента сложна и биты "разбросаны" по 8 байтам. Макрос упрощает создание дескриптора, автоматически формируя правильную битовую структуру из понятных параметров (база, лимит, атрибуты).

**Q: Почему программа "падает" при неправильном селекторе?**
A: В защищённом режиме процессор проверяет права доступа при каждой загрузке сегментного регистра. Если селектор некорректен (индекс за пределами таблицы, неправильный RPL) или дескриптор запрещает операцию (например, запись в кодовый сегмент), процессор генерирует исключение (обычно #GP - General Protection Fault). Если обработчик исключений не установлен (как в нашей лабораторной), происходит "тройной сбой" (Triple Fault) и перезагрузка.
